					 ...
					 dma_addr_t iova)
{
	...
	vpn2_idx = (iova >> PGDIR39_SHIFT) & PAGE_TABLE_LEVEL_MASK;
...
static int xt_iommu_domain_td_s1(struct xt_iommu_domain *xt_domain, int free)
{
	int ret;
	u64 *page_table;
...
static struct iommu_domain *xt_iommu_domain_alloc(unsigned type)
{
	...
	xt_iommu_domain_td_s1(xt_domain, 0);
...
static void xt_iommu_domain_free(struct iommu_domain *domain)
{
	...
	xt_iommu_domain_td_s1(xt_domain, 1);
...
static struct iommu_ops xt_iommu_ops = {
	...
	.default_domain_ops = &(const struct iommu_domain_ops) {
		...
		.map_pages	= xt_iommu_map,
		.unmap_pages	= xt_iommu_unmap,
		.iova_to_phys	= xt_iommu_iova_to_phys,
		.free		= xt_iommu_domain_free,
	}
};
...
static int xt_iommu_device_probe(struct platform_device *pdev)
{
	...
	ret = iommu_device_register(&xt_iommu->iommu, &xt_iommu_ops, dev);
	if (ret)
		return ret;
